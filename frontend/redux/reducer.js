...
export default function reducer(state = initialState, action) {
    switch (action.type) {
        ...
        case SET_FAVORITE_MOVIES:   return { ...state, favoriteMovies: action.payload };
        case ADD_FAVORITE:          return { ...state, favoriteIds: [...state.favoriteIds, Number(action.payload)] };
        case ADD_FAVORITE_MOVIE:
            if (state.favoriteMovies.some(m => Number(m.id) === Number(action.payload.id)))
                return state;
            return { ...state, favoriteMovies: [...state.favoriteMovies, action.payload] };
        case REMOVE_FAVORITE:       return {...state, favoriteIds:    state.favoriteIds.filter(id => id !== Number(action.payload)), favoriteMovies: state.favoriteMovies.filter(m => Number(m.id) !== Number(action.payload)), };
